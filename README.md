...
> **Status:** the package is `0.8.0`, and there is no production `v1.0.0` release yet. The managed `1.0.0` updater and its disposable operation harness are implemented. The production dependency audit reports no critical or high advisories. The moderate ones left are all one advisory against `@tiptap/core` (GHSA-cp6q-959q-f8rh), whose fix TomeCMS already has: it was backported to the 2.27.3 this repository pins, and the advisory still names only 3.30.4 as patched, so every 2.x release is reported. The integration, browser and operations gates run green against Astro 7. Two gates stay open because they need infrastructure this repository cannot stand up for itself: verifying a public, immutable release and its attestations, and accepting a real HTTPS VPS on `amd64` and `arm64`. Release notes: [0.8.0](docs/releases/0.8.0.md), [0.7.0](docs/releases/0.7.0.md), [0.6.0](docs/releases/0.6.0.md), [0.5.0](docs/releases/0.5.0.md), [0.4.0](docs/releases/0.4.0.md), [0.3.0](docs/releases/0.3.0.md), [0.2.0](docs/releases/0.2.0.md), and the [planned 1.0.0 boundary](docs/releases/1.0.0.md). The [changelog](CHANGELOG.md) sums up every version in one place.
...
The application's migrations are applied with `npm run db:migrate`, and the deploy helper runs them for you. An installation created from 0.7.0 or 0.6.0 has two waiting, `021_media_documents` and `022_navigation_new_tab`. One created from 0.5.0 or 0.4.0 has three, those and `020_site_brand`. One created from 0.3.0 has six, and one from before 0.3.0 has those and nine more; see the [0.8.0](docs/releases/0.8.0.md#upgrading), [0.6.0](docs/releases/0.6.0.md#upgrading), [0.4.0](docs/releases/0.4.0.md#upgrading) and [0.3.0](docs/releases/0.3.0.md#upgrading) notes. The admin names any migration it finds unapplied at the top of every screen.
