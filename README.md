...
The application's migrations are applied with `npm run db:migrate`, and the deploy helper runs them for you. An installation created from 0.6.0 has none waiting. One created from 0.5.0 or 0.4.0 has one, `020_site_brand`. One created from 0.3.0 has four, and one from before 0.3.0 has those and nine more; see the [0.6.0](docs/releases/0.6.0.md#upgrading), [0.4.0](docs/releases/0.4.0.md#upgrading) and [0.3.0](docs/releases/0.3.0.md#upgrading) notes. The admin names any migration it finds unapplied at the top of every screen.
