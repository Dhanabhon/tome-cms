...
- `sitePopup` returns a heading, words, one link, and a picture from the library by its id. The core checks the link the same way, and draws the picture only while it is still a ready image in the owner's library.
