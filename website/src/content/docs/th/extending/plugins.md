...
แกนระบบวาดแถบและป๊อปอัปก็จริง แต่โค้ดฝั่งเบราว์เซอร์ที่เปิดป๊อปอัปและปิดแถบไม่ได้อยู่ในแกนระบบ โค้ดนี้อยู่ใน `src/plugins/popup/client.ts` กับ `src/plugins/notice/client.ts` และแต่ละไฟล์โหลดผ่าน `publicClient` ของปลั๊กอินตัวนั้นเท่านั้น ปลั๊กอินที่คุณเขียนเองซึ่งคืน `sitePopup` หรือคืน `siteNotice` ที่มี key จึงต้องคืน `publicClient` สำหรับหน้าเดียวกันด้วย และมี `client.ts` ที่ทำงานส่วนนี้ ถ้าไม่มี ป๊อปอัปจะถูกวาดไว้แต่ไม่เคยเปิดขึ้นมา และปุ่มปิดของแถบจะกดแล้วไม่เกิดอะไรขึ้น
