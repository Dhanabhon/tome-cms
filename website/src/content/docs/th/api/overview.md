...
API ใช้ได้ทั้งโหมด bundled และโหมด headless ในโหมด headless (`TOME_CMS_FRONTEND_MODE=headless`) TomeCMS จะเลิกแสดงหน้าเว็บสาธารณะของตัวเอง และ API จะเป็นทางที่เนื้อหาของคุณไปถึงผู้อ่าน
...
เซิร์ฟเวอร์เซ็น cursor ทุกตัวและผูกไว้กับ query ที่สร้างมันขึ้นมา ถ้าใช้ cursor เดิมแต่เปลี่ยน `limit` หรือ `category` หรือแก้ cursor เอง จะได้ `400` พร้อมข้อความ `Invalid pagination cursor.` ลายเซ็นนี้ใช้ `TOME_CMS_CONTEXT_SECRET` ถ้าเปลี่ยนค่านี้ cursor ทุกตัวที่ออกไปก่อนหน้าจะใช้ไม่ได้อีก
...
API นี้ไม่มีอะไรให้ยืนยันตัวตน จึงไม่ต้องส่ง credentials ถ้าเรียกด้วย `credentials: 'include'` เบราว์เซอร์จะไม่รับคำตอบที่เป็น wildcard
...
preview เป็นของส่วนตัว คำตอบมี `Cache-Control: private, no-store` และ `Referrer-Policy: no-referrer` และไม่มี header CORS เลย เบราว์เซอร์จาก origin อื่นจึงอ่านไม่ได้ ให้ดึง preview จากเซิร์ฟเวอร์ของเว็บคุณ ไม่ใช่จากเบราว์เซอร์ของผู้อ่าน route ของ preview ไม่อยู่ในเอกสาร OpenAPI เพราะเอกสารนั้นอธิบายเฉพาะส่วนที่เป็นสาธารณะ

## สัญญาของ API (contract)

`/api/v1/content/openapi.json` คือสัญญาของ API เป็นเอกสาร OpenAPI 3.1 ที่บอกทุก route สาธารณะ พร้อมพารามิเตอร์และคำตอบของแต่ละ route เอกสารนี้สร้างจากโค้ดชุดเดียวกับที่ตอบคำขอ จึงตรงกับระบบที่ให้บริการมันเสมอ ใช้สร้าง client แบบมี type หรือเปิดอ่านเพื่อดูว่าเวอร์ชันของคุณรองรับอะไรบ้าง
