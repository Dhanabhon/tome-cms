...
Two themes ship with TomeCMS, `paper` and a sparer one called `plain`. You choose between them under "Appearance", in a preview that draws your own posts. Each language has its own home page slides, with a picture, a few words and a button, which the theme shows as a still banner or as a slider.
