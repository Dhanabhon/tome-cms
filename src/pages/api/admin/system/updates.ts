...
import { updateActionSchema } from '../../../../server/update/admin';
...
export const GET: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    return Response.json(await getUpdateStatus(), { headers: headers(requestId) });
  } catch (error) {
...
export const POST: APIRoute = async ({ clientAddress, request }) => {
  ...
  try {
    ...
    if (action.action === 'check') return Response.json(await refreshUpdateStatus(), { headers: headers(requestId) });
