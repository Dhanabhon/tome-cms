...
export const onRequest = defineMiddleware(async ({ redirect, url }, next) => {
  const { pathname } = url;
  if (SETUP_PATHS.has(pathname) || pathname === '/api/auth' || pathname.startsWith('/api/auth/') || pathname === '/health/live' || pathname === '/health/ready' || pathname.startsWith('/_astro/') || pathname === '/favicon.svg') {
    return next();
  }
