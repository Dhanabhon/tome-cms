...
/**
 * The admin says when the database is behind the build.
 *
 * On 2026-09-20 a migration was written and not run, so the code wrote to a column that was
 * not there and the owner met it as "The request could not be completed." on save, with
 * nothing anywhere saying why. /health/ready knew the whole time -- it checks for pending
 * migrations -- and nothing the owner looks at was asking it.
 *
 * This removes an applied migration's row, which is what a build ahead of its database
 * looks like from inside, and asks whether the admin says so before anything is saved.
 */

test.use({ stack: 'admin-migration-warning' });

const PROJECT = 'tomecms-migration-test';
const COMPOSE = ['compose', '-p', PROJECT, '-f', 'compose.test.yaml'];
const CREDENTIAL = 'migration-warning-secret-at-least-32-c';
...
test.beforeAll(async () => {
  ...
  serverEnv = {
    ...
    TOME_CMS_VITE_CACHE_DIR: 'node_modules/.vite-migration-warning',
  };
...
test.skip(
  ...
  'One browser is enough, and the virtual authenticator needs Chromium.',
);

test('the admin says when the database is behind the build', async ({ context, page }) => {
  ...
  const banner = page.locator('.admin-migrations');
  await expect(banner, 'a database that is up to date says nothing').toBeHidden();

  // What a build ahead of its database looks like from inside.
  const [last] = await sql<{ name: string }>`
    delete from kysely_migration where name = (select name from kysely_migration order by name desc limit 1) returning name
  `.execute(db).then((result) => result.rows);
  try {
    await page.goto(`${origin}/admin`);
    await expect(banner, 'the admin says so before anything is saved').toBeVisible();
    await expect(banner).toContainText(last.name);
    // On every screen, because it is true of every screen.
    await page.goto(`${origin}/admin/settings`);
    await expect(banner).toBeVisible();
  } finally {
    await sql`insert into kysely_migration (name, timestamp) values (${last.name}, ${new Date().toISOString()})`.execute(db);
  }

  await page.goto(`${origin}/admin`);
  await expect(banner, 'and stops saying it once the database catches up').toBeHidden();
});
