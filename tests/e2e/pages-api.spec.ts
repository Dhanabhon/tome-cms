...
test('requires authentication and rejects malformed or server-owned fields', async ({ page, request }) => {
  ...
  for (const response of [
    ...
    await request.post('/api/pages', { data: pageBody('Unauthorized') }),
    await request.put('/api/pages', { data: { ...pageBody('Unauthorized'), id } }),
    await request.patch('/api/pages', { data: { id, status: 'draft' } }),
    await request.delete(`/api/pages?id=${id}`),
  ]) {
  ...
  try {
    ...
    const malformed = await page.request.post('/api/pages', {
    ...
    const strictPut = await page.request.put('/api/pages', {
      data: { ...pageBody('Strict PUT'), id, owner_id: owner.id },
    });
    expect(strictPut.status()).toBe(400);
    const strictPatch = await page.request.patch('/api/pages', {
...
test('creates sanitized drafts, generates slugs, and rejects reserved or duplicate slugs', async ({ page }) => {
  ...
  try {
    ...
    const duplicate = await page.request.post('/api/pages', {
...
test('creates only owner-linked translation editions', async ({ page }) => {
  ...
  try {
    await signInAdmin(page, owner);
    await expect(page.getByRole('link', { name: 'New post' })).toBeVisible();
    const sourceResponse = await page.request.post('/api/pages', {
    ...
    const translationResponse = await page.request.post('/api/pages', {
      data: {
    ...
    const sameLocale = await page.request.post('/api/pages', {
      data: {
    ...
    const foreignResponse = await page.request.post('/api/pages', {
      data: {
...
test('scopes CRUD to the owner and preserves the first publication timestamp', async ({ page }) => {
  ...
  try {
    ...
    const listed = await page.request.get('/api/pages');
    expect(listed.status()).toBe(200);
    expect((await listed.json()).pages.map(({ id }: Page) => id)).toEqual([owned.id]);

    for (const response of [
      await page.request.put('/api/pages', { data: { ...pageBody('Foreign update'), id: foreignPage.id } }),
      await page.request.patch('/api/pages', { data: { id: foreignPage.id, status: 'published' } }),
      await page.request.delete(`/api/pages?id=${foreignPage.id}`),
    ]) {
      expect(response.status()).toBe(404);
    }

    const emptyCreate = await page.request.post('/api/pages', {
    ...
    const emptied = await page.request.put('/api/pages', {
      data: { ...pageBody('Empty draft', owned.slug), id: owned.id, contentJson: { type: 'doc', content: [] }, contentHtml: '' },
    });
    expect(emptied.status()).toBe(200);
    const rejectedPublish = await page.request.patch('/api/pages', {
    ...
    const updated = await page.request.put('/api/pages', {
    ...
    expect((await page.request.patch('/api/pages', { data: { id: owned.id, status: 'draft' } })).status()).toBe(200);
    const republished = await page.request.patch('/api/pages', { data: { id: owned.id, status: 'published' } });
    expect(republished.status()).toBe(200);
    expect(((await republished.json()).page as Page).published_at).toBe(firstPublishedAt);

    const removed = await page.request.delete(`/api/pages?id=${owned.id}`);
    expect(removed.status()).toBe(204);
    expect((await page.request.get('/api/pages').then((response) => response.json())).pages).toEqual([]);
  } finally {
    await admin.from('pages').delete().in('author_id', [owner.id, foreign.id]);
    await deleteOwner(owner);
    await deleteOwner(foreign);
...
test('PATCH does not overwrite a page autosaved after its publication check', async ({ page }, testInfo) => {
  ...
  try {
    ...
    const created = await page.request.post('/api/pages', { data: draft });
    expect(created.status()).toBe(201);
    const saved = (await created.json()).page as Page;
    const { PATCH } = await vite.ssrLoadModule('/src/pages/api/pages/index.ts');
    let autosaved = false;
    globalThis.fetch = async (input, init) => {
      ...
      if (!autosaved && (init?.method ?? 'GET') === 'GET' && url.pathname === '/rest/v1/pages' && url.searchParams.get('id') === `eq.${saved.id}`) {
        autosaved = true;
        const update = await page.request.put('/api/pages', {
    ...
    const cookie = (await page.context().cookies()).map(({ name, value }) => `${name}=${value}`).join('; ');
    const request = new Request(new URL('/api/pages', page.url()), {
